...
//-----------------------------------------------------------------------
//----------------------------execute commands---------------------------
    string toBeSearched_fieldName, toBeSearched_valueStr, comparison_valueStr;
    ...
    for(i = 0; i < commands.size(); i++) //throw wrong command exceptions by invalidity of commands
    {
        if(commands[i].get_isValid()) //execute command if only its a valid command
        {
            if(commands[i].getOperation() == "search") //if command is search
            {
                ...
                toBeSearched_fieldName = commands[i].get_field_name(); //take the to be searched file name
                ...
                {
                    if(fields[j].getName() == toBeSearched_fieldName) 
                        break;
                }
                ...
                if(fields[j].getDim() == SINGLE) //if field is single look at just its [k][j][0] indiced elements
                {
                    ...
                    {   
                        ...
                        const Entry<string>* stringEntryPtr = dynamic_cast<const Entry<string>*>(entries[k][j][0]); //try to cast EntryBase* to Entry<string>* to be able to compare
                        const Entry<double>* dblEntryPtr = dynamic_cast<const Entry<double>*>(entries[k][j][0]); //try to cast EntryBase* to Entry<string>* to be able to compare
                        const Entry<int>* intEntryPtr = dynamic_cast<const Entry<int>*>(entries[k][j][0]); //try to cast EntryBase* to Entry<string>* to be able to compare

                        if (stringEntryPtr) //if string* cast is succesful
                        {
                        ...
                        else if (dblEntryPtr) //if double* cast is succesful
                        {
                        ...
                        else if (intEntryPtr) //if int* cast is succesful
                        {
                ...
                /*else if(fields[j].getType() == MULTI) //if field is multi look at all elements vertically
                {
                    for(k = 0; k < entries.size(); k++)
                    {
                        Entry<string>* stringEntryPtr = dynamic_cast<Entry<string>*>(entries[k][j][0]); //cast EntryBase* to Entry<string>* to be able to compare

                        if (stringEntryPtr) //if cast is succesful
                        {
                            comparison_value = stringEntryPtr->getValueTyped();
                            
                            if (comparison_value == toBeSearched_value) 
                            {
...
unordered_map<string, int> findDuplicatedEntries(const vector<string>& vec_entries)
{
    unordered_map<string, int> entryCounts; //to hold num of occurences of every element

    //increment num of occurence when a string is found
    for (const string& entry : vec_entries) 
    {
        entryCounts[entry]++;
    }
    ...
    for (const auto& pair : entryCounts) 
    {
        if (pair.second > 1) 
        {
            std::cout << pair.first << " num of occurence: " << pair.second << std::endl;
        }
    }
