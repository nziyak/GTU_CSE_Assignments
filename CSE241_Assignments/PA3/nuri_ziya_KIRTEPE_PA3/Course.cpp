...
namespace PA3
{
    ...
    #endif
    Course::Course(string course_name, string course_code) : name(course_name), code(course_code), student_counter(0), student_rfr_size(0)
    { 
        student_references = nullptr; 
    }

    Course::Course() : student_counter(0), student_rfr_size(0)
    {
        student_references = nullptr; 
    }

    Student*** Course::get_student_references()
    { 
        return &student_references; 
    }

    Student** Course::get_student_referencess()
    {
        return student_references; 
    }

    ostream& operator<<(ostream& os, const Course toBePrintedCourse)
    {
        os << toBePrintedCourse.code << " " << toBePrintedCourse.name << endl;
        return os;
    }

    int Course::find_student_index(string name, string id)
    {
